...
Worker 故障、断网、CORS 失败或 Push 服务错误不得回滚本地任务。共享 API 请求有 15 秒上限，并在创建空间时立即显示加载状态、防止重复提交；超时或网络错误必须给出可见提示。应用打开、`online` 事件和设置页“立即分享”触发一次轻量重试；不引入通用 outbox、指数重放或跨设备合并器。
...
- 主场景 `assets/scene.webp`、公告板位置和羊皮纸/木质语言保持为视觉锚点；烟雾、飞鸟、蓝紫蝴蝶、飘叶、微光和爱心只使用独立 CSS 覆盖层，不移动主图或交互覆盖层，并遵守 `prefers-reduced-motion`，设置页会说明系统是否因此暂停动画。
