...
- PWA 新版本提示；
- 时间、季节、天气的轻量氛围、可关闭的场景小动画、天气显示、完成动画和极轻植物成长；
...
- 季节氛围变化；
- 天气效果开关；
- 场景小动画开关（手机上可直接辨认的偶发爱心、烟囱淡烟、飞鸟、蓝紫蝴蝶、飘叶和草地微光；系统要求减少动态时停用并说明原因）；
- 完成动画开关；
- BGM 开关与音量；
...
PWA 必须监听注册对象的异步 `updatefound`；关键入口脚本与样式使用发布版本标识，避免旧 Worker 控制页面时首屏仍取到上一版静态资源。

## 13. 验收标准

### 产品行为

- [ ] 首页显示已经开始的开放任务；开始日期晚于今天的任务在未来列表中可见。
- [ ] 逾期、今日到期和其他已开始任务的稳定排序可由固定测试数据复现。
...
- [ ] 场景动画在手机尺寸下可辨认；系统开启“减少动态效果”时，设置页说明动画暂停原因。
- [ ] BGM 在每台设备独立开启，移动浏览器的首次点击能直接启动或恢复播放，失败时提供可见重试入口。
